...
export async function initDb() {
  await query(`
    CREATE TABLE IF NOT EXISTS users (
      ...
      theme_color VARCHAR(60) NOT NULL DEFAULT 'grad:#7c3aed,#4f7dff',
  ...
  await query(`ALTER TABLE users ALTER COLUMN theme_color SET DEFAULT 'grad:#7c3aed,#4f7dff'`);
  ...
  // Settings > Privacy & Security > "Who can see my profile" style Private Profile flag.
  // Profile info itself always stays visible; this only gates who can start a NEW direct
  // chat without going through the request flow (see chat_requests below).
  await query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS is_private SMALLINT NOT NULL DEFAULT 0`);

  // Per-user notification mute list. Muting someone stops notifications generated by
  // their activity (messages, requests) from reaching the muter — it does NOT block
  // them, they can still message/request normally, the muter just isn't notified.
  await query(`
  ...
  // Chat requests: required to start a direct conversation with a private-profile user.
  // Only one active ("pending") request per direction is allowed at a time.
  await query(`
  ...
  `);
  await query(
    `CREATE UNIQUE INDEX IF NOT EXISTS chat_requests_pending_uidx
     ON chat_requests(requester_id, target_id) WHERE status = 'pending'`
  );

  // In-app notification feed (chat requests, request accepted, etc). Shown in the
  // sidebar Notifications panel with an unread-count badge.
  await query(`
...
function toPublicUser(row) {
  return {
    ...
    themeColor: row.theme_color ?? row.themeColor ?? "grad:#7c3aed,#4f7dff",
...
export async function createUser(phoneNumber, username, passwordHash) {
  ...
  const [rows] = await query(
    ...
    [phoneNumber, username, passwordHash, "grad:#7c3aed,#4f7dff"]
  );
  return toPublicUser({
    id: rows[0].id,
    ...
    avatar_url: null,
    tagline: null,
    theme_color: "grad:#7c3aed,#4f7dff",
    show_online: 1,
  });
}
...
export async function setPrivateProfile(userId, isPrivate) {
  await query("UPDATE users SET is_private = ? WHERE id = ?", [isPrivate ? 1 : 0, userId]);
}
...
// True if EITHER side has blocked the other — used to fully stop messages/requests/
// notifications flowing in either direction between a blocker and a blocked user.
...
// --- Muted users (Settings/notifications — mute without blocking) ---

export async function muteUser(muterId, mutedId) {
  await query(
    "INSERT INTO muted_users (muter_id, muted_id) VALUES (?, ?) ON CONFLICT DO NOTHING",
    [muterId, mutedId]
  );
}

export async function unmuteUser(muterId, mutedId) {
  await query("DELETE FROM muted_users WHERE muter_id = ? AND muted_id = ?", [muterId, mutedId]);
}

export async function isMuted(muterId, mutedId) {
  const [rows] = await query(
    "SELECT 1 FROM muted_users WHERE muter_id = ? AND muted_id = ?",
    [muterId, mutedId]
  );
  return rows.length > 0;
}

export async function listMutedUsers(muterId) {
  const [rows] = await query(
    `SELECT u.id, u.username, u.avatar_url AS "avatarUrl"
     FROM muted_users m JOIN users u ON u.id = m.muted_id
     WHERE m.muter_id = ? ORDER BY u.username`,
    [muterId]
  );
  return rows;
}

// --- Direct-conversation existence check (used to let already-connected pairs keep
// chatting even if one of them later turns on Private Profile) ---

export async function directConversationExists(userIdA, userIdB) {
  const [rows] = await query(
...
// --- Chat requests (Private Profile flow) ---

export async function getPendingRequestBetween(requesterId, targetId) {
  const [rows] = await query(
    "SELECT * FROM chat_requests WHERE requester_id = ? AND target_id = ? AND status = 'pending'",
...
export async function getChatRequestById(id) {
  const [rows] = await query("SELECT * FROM chat_requests WHERE id = ?", [id]);
  return rows[0] || null;
}

export async function updateChatRequestStatus(id, status) {
  const [rows] = await query(
    `UPDATE chat_requests SET status = ?, responded_at = NOW() WHERE id = ? RETURNING *`,
    [status, id]
  );
  return rows[0] || null;
}
...
export async function listIncomingRequests(userId) {
  const [rows] = await query(
    `SELECT r.id, r.status, r.created_at AS "createdAt",
            ...
            u.avatar_url AS "requesterAvatarUrl", u.tagline AS "requesterTagline"
...
export async function countPendingRequests(userId) {
  const [rows] = await query(
    "SELECT COUNT(*)::int AS count FROM chat_requests WHERE target_id = ? AND status = 'pending'",
    [userId]
  );
...
// Creates an in-app notification for `userId`, unless `userId` has muted `fromUserId`
// or has blocked/been-blocked-by them (in which case it's silently skipped).
export async function createNotification(userId, type, fromUserId, data = null) {
  if (fromUserId) {
    if (await isMuted(userId, fromUserId)) return null;
    if (await isBlockedEitherWay(userId, fromUserId)) return null;
  }
  const [rows] = await query(
    `INSERT INTO notifications (user_id, type, from_user_id, data) VALUES (?, ?, ?, ?) RETURNING *`,
    [userId, type, fromUserId || null, data ? JSON.stringify(data) : null]
  );
  return rows[0];
}
...
export async function listNotifications(userId, limit = 50) {
  const [rows] = await query(
    `SELECT n.id, n.type, n.data, n.read, n.created_at AS "createdAt",
            u.id AS "fromUserId", u.username AS "fromUsername", u.avatar_url AS "fromAvatarUrl"
...
export async function countUnreadNotifications(userId) {
  const [rows] = await query(
    "SELECT COUNT(*)::int AS count FROM notifications WHERE user_id = ? AND read = 0",
    [userId]
  );
  return rows[0]?.count || 0;
}
